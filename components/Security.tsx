      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="flex flex-col lg:flex-row gap-32 items-center">
          <div className="lg:w-1/2 reveal">
            ...
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-[88px] font-brand font-bold text-brand-primary uppercase mb-16 leading-[0.85] tracking-tighter">
            ...
            <div className="grid sm:grid-cols-2 gap-x-12 gap-y-16">
              {[
                ...
                { icon: <Terminal />, title: "Cifrado Seguro", desc: "Protección TLS en tránsito y cifrado de alta seguridad para datos en reposo." }
              ].map((item, i) => (
          ...
          <div className="lg:w-1/2 reveal flex justify-center" style={{ transitionDelay: '0.2s' }}>
             <div className="w-full aspect-square max-w-lg bg-white rounded-[32px] md:rounded-[48px] shadow-2xl border border-brand-border flex items-center justify-center relative group overflow-hidden mx-4 md:mx-0">
                <div className="absolute inset-0 bg-brand-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
                <div className="w-2/3 aspect-square border-2 border-brand-border rounded-[24px] md:rounded-[40px] flex items-center justify-center relative group-hover:scale-110 transition-transform duration-700">
                   <div className="w-1/2 aspect-square bg-brand-primary rounded-2xl md:rounded-3xl flex items-center justify-center shadow-[0_30px_60px_-15px_rgba(15,47,74,0.5)]">
                      <Lock className="w-10 h-10 md:w-16 md:h-16 text-brand-accent" />
                   </div>
                   {/* Technical tags in Mono - hidden on small screens to prevent overflow */}
                   <div className="hidden sm:block absolute top-0 left-0 bg-white border border-brand-border px-2 md:px-3 py-1 md:py-1.5 font-mono font-bold text-[7px] md:text-[8px] -translate-x-1/4 md:-translate-x-1/2 -translate-y-1/2 text-brand-primary">LAYER_01_SEC</div>
                   <div className="hidden sm:block absolute bottom-0 right-0 bg-white border border-brand-border px-2 md:px-3 py-1 md:py-1.5 font-mono font-bold text-[7px] md:text-[8px] translate-x-1/4 md:translate-x-1/2 translate-y-1/2 text-brand-primary">ENCRYPT_V4</div>
                </div>
                {/* Visual grid overlay */}
                <div className="absolute inset-0 p-6 md:p-12 opacity-[0.03] pointer-events-none">
